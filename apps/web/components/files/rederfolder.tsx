...
import { templateFileComparator } from "@/lib/utils";
export const RenderFolder = ({
  ...
  const [createFileFolderValue, setCreateFileFolderValue] = useState<
    null | TemplateFile | TemplateFolder
  >(null);

  const componentRef = useRef<HTMLDivElement>(null);
  const sortedItems = useMemo(() => {
    return [...folder.items].sort(templateFileComparator);
  }, [folder.items]);
            ...
            <FileTree
              key={
                "folderName" in item
                  ...
                  : `file-${path}/${item.fileName}.${item.fileExtension}`
              }
